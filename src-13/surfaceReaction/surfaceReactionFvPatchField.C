...
void Foam::surfaceReactionFvPatchField::map
(
    const fvPatchField<scalar>& ptf,
    const fieldMapper& m
)
{
    RobinFvPatchScalarField::map(ptf, m);

    const surfaceReactionFvPatchField& mptf =
        refCast<const surfaceReactionFvPatchField>(ptf);

    m(RobinKeff_, mptf.RobinKeff_);
    m(RobinFeff_, mptf.RobinFeff_);
}

void Foam::surfaceReactionFvPatchField::reset
(
    const fvPatchField<scalar>& ptf
)
{
    RobinFvPatchScalarField::reset(ptf);

    const surfaceReactionFvPatchField& mptf =
        refCast<const surfaceReactionFvPatchField>(ptf);

    RobinKeff_ = mptf.RobinKeff_;
    RobinFeff_ = mptf.RobinFeff_;
}
